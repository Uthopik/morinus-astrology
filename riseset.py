...
class RiseSet:
	...
	def __init__(self, jd, cal, lon, lat, alt, pls, diff): # <--- Añadimos diff aquí
		self.jd = jd
		self.cal = cal
		self.lon = lon
		self.lat = lat
		self.alt = alt
		self.diff = diff # <--- Guardamos el valor real (ej. 1.0 o 2.0)

		self.calflag = astrology.SE_GREG_CAL
		if self.cal == chart.Time.JULIAN:
			self.calflag = astrology.SE_JUL_CAL

		# La mejor forma: convertir las horas de 'diff' a fracción de día juliano
		self.offs = self.diff / 24.0  
...
#		self.printRiseSet(pls)
	...
	def calcTimes(self):
		...
		for i in range(planets.Planets.PLANETS_NUM):#Nodes are excluded
			...
			# Añade esto para espiar los datos en la terminal:
			#print(f"DEBUG: Planeta={i}, JD={self.jd}, Lat={self.lat}, Lon={self.lon}")
			#Rise
