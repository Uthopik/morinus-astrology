...
class ProfectionsWnd(commonwnd.CommonWnd):
	...
	def drawBkg(self):
		...
		for i in range(self.COLUMN_NUM):
			...
			if i > 3:
				clr = tclr
			draw.text((BOR+offs+(cols[i]-w)//2, BOR+2*self.LINE_HEIGHT+(self.TITLE_HEIGHT-h)//2), txt_list[i], fill=clr, font=fnt)
			offs += cols[i]
	...
	def drawline(self, draw, x, y, clr, pcharts, age, idx):
		...
		for i in range(self.COLUMN_NUM):
			...
			if i == ProfectionsWnd.AGE:
				...
				elif i == ProfectionsWnd.MC:
					lon = pcharts[0].houses.ascmc[houses.Houses.MC]
				elif i == ProfectionsWnd.SUN:
				...
				elif i == ProfectionsWnd.FORTUNE:
					lon = pcharts[0].fortune.fortune[fortune.Fortune.LON]
				elif i >= ProfectionsWnd.MERCURY:
				...
				pos_val = d%chart.Chart.SIGN_DEG
				
				# Medición de los 3 componentes para el centrado
				...
				txt_pos = (str(pos_val)).rjust(2)+self.deg_symbol+(str(m)).zfill(2)+"'"+(str(s)).zfill(2)+'"'
				b_pos = draw.textbbox((0, 0), txt_pos, font=self.fntText)
				w, h = b_pos[2]-b_pos[0], b_pos[3]-b_pos[1]
				
				offset = (offs[i]-(w+wsp+wsg))//2
				draw.text((x+summa+offset, y+(self.LINE_HEIGHT-h)//2), txt_pos, fill=tclr, font=self.fntText)
				draw.text((x+summa+offset+w+wsp, y+(self.LINE_HEIGHT-hsg)//2), self.signs[sign_idx], fill=tclr, font=self.fntMorinus)
