...
# ###########################################
	...
	def showFindTime(self, bc, fnd, arplac):
		...
		obl, serr = astrology.swe_calc(time.jd+d, astrology.SE_ECL_NUT, 0)
...
#				self.calc()##
	...
	def subLoad(self, fpath, dpath, dontclose = False):
		...
		try:
			...
			place_obj = chart.Place(place, deglon, minlon, 0, east, deglat, minlat, seclat, north, altitude)
			time_obj = chart.Time(year, month, day, hour, minute, second, bc, cal, zt, plus, zh, zm, daylightsaving, place_obj)
			
			# Aquí es donde se reconstruye la carta
			chrt = chart.Chart(name, male, time_obj, place_obj, htype, notes, self.options)

		except Exception as e:
			# Solo entramos aquí si algo falló realmente
			# El mensaje ahora incluirá el error técnico para saber qué pasa
			error_msg = f"{mtexts.txts['FileError']}\nDetalle: {str(e)}"
			dlgm = wx.MessageDialog(self, error_msg, mtexts.txts['Error'], wx.OK|wx.ICON_EXCLAMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
	...
	def save(self):
		dlg = wx.FileDialog(self, mtexts.txts['SaveHor'], '', self.horoscope.name, mtexts.txts['HORFiles'], wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
		...
		if dlg.ShowModal() == wx.ID_OK:
			...
			try:
				# Abrimos en modo binario para Python 3
				...
				dlg_err = wx.MessageDialog(self, mtexts.txts['FileError'], mtexts.txts['Error'], wx.OK|wx.ICON_EXCLAMATION)
				dlg_err.ShowModal()
				dlg_err.Destroy()
	...
	def onSaveAsBitmap(self, event):
		...
		name = self.horoscope.name if self.horoscope.name != '' else mtexts.txts['Horoscope']
		...
		if dlg.ShowModal() == wx.ID_OK:
			...
			# --- RESOLUCIÓN FIJA DE SALIDA (Tu estándar) ---
			TARGET_W = 1848.0
			
			# 1. Medimos cómo está la ventana ahora para sacar el Ratio
			size = self.GetClientSize()
			orig_w, orig_h = float(size.width), float(size.height)
			ratio = orig_h / orig_w
			
			# 2. Calculamos la altura proporcional para el Target
			target_h = int(TARGET_W * ratio)
			
			# 3. Guardamos el tamaño visual para restaurar
			old_size = self.GetClientSize()

			# 4. CAMBIO TEMPORAL AL ESTÁNDAR 1848x...
			self.SetClientSize((int(TARGET_W), target_h))
			self.drawBkg()
			...
			try:
				...
				self.sw.buffer.SaveFile(fpath, wx.BITMAP_TYPE_BMP)
			
			# 6. RESTAURAMOS la ventana a como la tenías antes
			self.SetClientSize(old_size)
			self.drawBkg()
			self.Refresh()
			
			self.fpathimgs = dlg.GetDirectory()
...
# ###################################      
	...
	def onExactTransits(self, event):
		...
		# SI ES ANTES DE CRISTO: Mostramos aviso y salimos
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
			return # Aquí sí tiene sentido el return

		# SI NO ES B.C. (Caso normal): Continuamos con el resto
		if self.trmondlg == None:
	...
	def onProfections(self, event):
		...
		# SI ES ANTES DE CRISTO: Mostramos aviso y salimos correctamente
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
		...
		# SI NO ES B.C.: Continuamos con el código normal
...
# ###################################
	...
	def onPrimaryDirs(self, event):
		...
		# --- PARCHE DE SEGURIDAD PARA EL ERROR DLGM ---
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
			return # Solo salimos si es BC
		# ----------------------------------------------
		...
		if val == wx.ID_OK:
			...
			# Nota para AUR: En Python 3, thread suele ser _thread
			import _thread as thread
	...
	def OnPDReady(self, event):
		...
		if self.abort.abort:
			...
			if self.pds != None and len(self.pds.pds) > 0:
				...
				# Solo creamos y mostramos el diálogo si NO hay direcciones
	...
	def onTransits(self, event):
		# Importamos aquí arriba para que TODA la función lo vea
		import mtexts
		import wx

		# 1. Seguridad inicial: Si no hay carta abierta, salimos
		if self.horoscope == None:
			return

		# 2. Manejo de barra de estado en Windows
		if wx.Platform == '__WXMSW__' and not self.splash:
			self.handleStatusBar(True)

		# 3. CORRECCIÓN DE INDENTACIÓN (El bloque del "Antes de Cristo")
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
			return # Salimos solo si es BC

		# 4. Ahora mtexts ya es conocido aquí abajo y no dará error
		if self.trdatedlg == None:
	...
	def onRevolutions(self, event):
		# Importamos mtexts y wx al inicio de la función para seguridad
		import mtexts
		import wx

		# 1. Manejo de barra de estado en Windows
		if wx.Platform == '__WXMSW__' and not self.splash:
			self.handleStatusBar(True)

		# 2. CORRECCIÓN: Tabulamos el aviso de "No Disponible" para AC
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
			return # Salimos solo si es antes de Cristo

		# 3. Abrir el diálogo de tipo de revolución
		if self.revdlg == None:
			import revolutionsdlg
		...
		if val == wx.ID_OK:	
			...
			if result:
				# ... (Lógica de cálculo de Solar/Lunar) ...
				...
				if self.options.ayanamsha != 0 and self.revdlg.typecb.GetCurrentSelection() == 0:
					t1, t2, t3, t4, t5, t6 = self.calcPrecNutCorrectedSolar(revs)

				import timespacedlg
				dlg = timespacedlg.TimeSpaceDlg(self, mtexts.txts['Revolutions'], self.options.langid)
				ti = (t1, t2, t3, t4, t5, t6, False, 0, 0, 0) # Simplificado para evitar conflictos
				dlg.initialize(self.horoscope, ti)	
				dlg.CenterOnParent()

				val = dlg.ShowModal()
				if val == wx.ID_OK:
					# ... (Lógica de creación de la carta de revolución) ...
					wait = wx.BusyCursor()
					direc = dlg.placerbE.GetValue()
					hemis = dlg.placerbN.GetValue()
					...
					if dlg.pluscb.GetCurrentSelection() == 1:
						plus = False
					time = chart.Time(t1, t2, t3, t4, t5, t6, False, self.horoscope.time.cal, 0, plus, 0, 0, False, place, False)
					...
					rw = transitframe.TransitFrame(self, self.title.replace(mtexts.typeList[self.horoscope.htype], mtexts.typeList[revtype]), revolution, self.horoscope, self.options)
					rw.Show(True)
				dlg.Destroy()
			
			else:
				# 4. CORRECCIÓN FINAL: Solo mostrar error si 'result' es False
	...
	def calcPrecNutCorrectedSolar(self, revs):
		...
		dat, serr = astrology.swe_calc_ut(JD1900, astrology.SE_ECL_NUT, 0)
		NutLon1900 = dat[2]
		SVP1900 = 360.0-FBAyanamsa1900-NutLon1900

		#calc natalprecfrom1900
		dat, serr = astrology.swe_calc_ut(self.horoscope.time.jd, astrology.SE_ECL_NUT, 0)
		NutLonNatal = dat[2]
		...
		while (DiffAngle > 0.00001):
			dat, serr = astrology.swe_calc_ut(jdSol, astrology.SE_SUN, pflag)
			TranSunLon = dat[0]
			TranSunVel = dat[3]

			dat, serr = astrology.swe_calc_ut(jdSol, astrology.SE_ECL_NUT, 0)
	...
	def onSunTransits(self, event):
		...
		# 1. CORRECCIÓN: Tabulamos para que solo ocurra si es BC
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
		...
		# 2. Manejo del diálogo de selección
		if self.suntrdlg == None:
			import suntransitsdlg
		...
		if val == wx.ID_OK:	
			...
			if result:
				...
				# 3. CORRECCIÓN FINAL: Tabulamos el error de "No se pudo calcular"
	...
	def onSecondaryDirs(self, event):
		...
		# 2. CORRECCIÓN: Tabulamos el aviso de "No Disponible" para fechas BC
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
			return # Ahora el return solo se ejecuta si la carta es BC

		# 3. Carga del diálogo de Direcciones Secundarias
		if self.secdirdlg == None:
			import secdirdlg
	...
	def onElections(self, event):
		...
		# 2. CORRECCIÓN: Tabulamos el bloque BC para que no bloquee el resto
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
		...
		# 3. Cálculo de la carta de Elecciones (ahora sí llegará aquí)
		...
		# 4. Apertura del frame de Elecciones
		...
		# 5. Apertura del Stepper de Elecciones
	...
	def onProfectionsChart(self, event):
		...
		# 2. CORRECCIÓN: El bloque BC ahora está bien cerrado
		if self.horoscope.time.bc:
			dlgm = wx.MessageDialog(self, mtexts.txts['NotAvailable'], '', wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
			return # Este return ahora SÓLO se ejecuta si es BC

		# 3. Inicialización del diálogo de Profecciones
		...
		if val == wx.ID_OK:
			...
			if self.options.zodprof:
				...
				if not self.options.usezodprojsprof and (y == self.horoscope.time.year or (y-self.horoscope.time.year) % 12 == 0) and m == self.horoscope.time.month and d == self.horoscope.time.day:
					...
					# Modificar posiciones de planetas y fortuna
					pchart.planets.calcMundaneProfPos(pchart.houses.ascmc2, pchartpls.planets.planets, self.horoscope.place.lat, self.horoscope.obl[0])
					pchart.fortune.calcMundaneProfPos(pchart.houses.ascmc2, pchartpls.fortune, self.horoscope.place.lat, self.horoscope.obl[0])
	
					pchart.calcAspMatrix()

			# Mostrar el Frame y el Stepper
			import common
		...
		if val == wx.ID_OK:	
			if(dlg.check(self.options)):
				...
				if self.options.autosave:
					# Primero guardamos el cambio del tema en el archivo
					if self.options.saveAppearance1():
						self.moptions.Enable(self.ID_SaveOpts, False)
						# ¡ESTA ES LA CLAVE!:
						# Como no existe updateTheme, forzamos a recargar
						# para que Morinus lea los nuevos colores del tema.
						self.options.load() 
	...
	def onSymbols(self, event):
		...
		if val == wx.ID_OK:	
			if dlg.check(self.options):
				...
				# Guardamos los símbolos siempre, sin preguntar al Autosave
				self.options.saveSymbols()
				self.moptions.Enable(self.ID_SaveOpts, False)
...
# ###########################################		
	...
	def onLanguages(self, event):
		...
		if val == wx.ID_OK:
			if dlg.check(self.options):
				# 1. Guardamos el cambio en el objeto opciones
				# 2. Forzamos el guardado físico en el archivo de configuración
				self.options.save() 
				
				# 3. Actualizamos los textos internos por si acaso
				mtexts.setLang(self.options.langid)

				# 4. Avisamos al usuario
				wx.MessageBox("Idioma guardado. Por favor, reinicia Morinus para ver los cambios.", "Info", wx.OK | wx.ICON_INFORMATION)
...
# ###########################################
	...
	def onHelp(self, event):
		...
		if not os.path.exists(fname):
			...
			# Estas tres líneas ahora están DENTRO del 'if'
			dlgm = wx.MessageDialog(self, txt, mtexts.txts['Error'], wx.OK|wx.ICON_INFORMATION)
			dlgm.ShowModal()
			dlgm.Destroy()
		else:
			# El 'else' ahora sí tiene un 'if' justo encima al que referirse
	...
	def onAbout(self, event):
		...
		# En wxPython 4.x usamos wx.adv
		info = wx.adv.AboutDialogInfo()
		
		# Cambiamos asignaciones directas por métodos Set...
		info.SetName(mtexts.txts['Morinus'])
		info.SetVersion('8.2.0')
		info.SetCopyright(mtexts.txts['FreeSoft'])
		info.SetDescription(mtexts.txts['Description'] + " (Swiss Ephemeris inside)")
		
		# La URL debe ser un solo string
		info.SetWebSite('https://sourceforge.net/p/morinus/')
		
		# Los desarrolladores se pasan como una lista de strings
		developers_list = [
			...
			'Elias Sagardia (Spain); josearrillaga@ik.me (modernization / Python 3 port)'
		]
		info.SetDevelopers(developers_list)
		
		info.SetLicense(mtexts.licensetxt)
		
		# Finalmente llamamos a AboutBox desde wx.adv
