...
class FixStarsAspectsWnd(commonwnd.CommonWnd):
	...
	def __init__(self, parent, chrt, options, mainfr, id = -1, size = wx.DefaultSize):
		...
		self.SPACE = int(self.FONT_SIZE/6)
...
# ###################################
		...
		# --- VARIABLES DE AJUSTE MANUAL ---
		OFFSET_ANGULOS = -12  # <--- Cambia esto para subir/bajar Asc, Dsc, MC, IC
		OFFSET_ORBE = -6     # <--- Cambia esto para subir/bajar el número del orbe (0.2, 0.4...)
		# ----------------------------------
		...
		for i in range(len(txts)):
			...
			bbox = draw.textbbox((0, 0), txts[i], font=self.fntSymbol)
			w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
			# Aplicamos OFFSET_ANGULOS aquí
			draw.text((x+(self.SQUARE_SIZE-w)//2, y+(self.SQUARE_SIZE-h)//2 + OFFSET_ANGULOS), txts[i], fill=txtclr, font=self.fntSymbol)
...
# ###################################
		...
		for i in range(num):
			...
			nombre_est = self.chart.fixstars.data[i][fixstars.FixStars.NOMNAME]
			if nombre_est in self.options.fixstars:
				orb = self.options.fixstars[nombre_est]
			else:
				orb = 1.0 
			...
			for j in range(4):
				...
				for numasp in runAspects:
					...
					if AspType and self.options.aspect[numasp]:
						...
						# CORRECCIÓN textsize -> textbbox
						bbox = draw.textbbox((0, 0), txt, font=self.fntAspects)
						w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
						...
						# CORRECCIÓN textsize -> textbbox
						bbox = draw.textbbox((0, 0), OrbDegree, font=self.fntTextOrb)
						w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
						xx = BOR+self.CELL_WIDTH+self.SPACE+j*(self.SQUARE_SIZE+self.SPACE)
						yy = BOR+self.TITLE_HEIGHT+self.SPACE+((self.SQUARE_SIZE-h)//1.7)+i*(self.SQUARE_SIZE+self.SPACE)+OFFSET_ORBE
		...
		for i in range(num):
			...
			if not self.bw:
				if self.options.useplanetcolors:
					...
					clr = self.clrs[dign]
			# CORRECCIÓN textsize -> textbbox
			bbox = draw.textbbox((0, 0), common.common.Planets[i], font=self.fntMorinus)
			w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
...
# ###################################
		...
		if not self.options.intables or (self.options.intables and self.options.showlof):
			self.drawSquare(draw, x, y, tableclr)
			# CORRECCIÓN textsize -> textbbox
			bbox = draw.textbbox((0, 0), common.common.fortune, font=self.fntMorinus)
			w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
			clr = (0,0,0)
			if not self.bw:
				if self.options.useplanetcolors:
...
# ###################################
		...
		if not self.options.intables or (self.options.intables and self.options.houses):
			for i in range(len(self.hidx)):
				...
				bbox = draw.textbbox((0, 0), txt_h, font=self.fntText)
				w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
				draw.text((x+(self.SQUARE_SIZE+self.SPACE)*i+(self.SQUARE_SIZE-w)//2, y+(self.SQUARE_SIZE-h)//2), txt_h, fill=(0,0,0), font=self.fntText)
