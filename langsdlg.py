...
class LanguagesDlg(wx.Dialog):
	...
	def check(self, opts):
		...
		# GetSelection() es más estándar en wxPython 4 (Phoenix)
		selid = self.langcb.GetSelection()
		
		# Verificamos que sea una selección válida (mayor o igual a 0)
		if selid != wx.NOT_FOUND:
			# MUY IMPORTANTE: Revisa si en tu morin.py usas 'langid' o 'lang'
			# Si el error persiste, prueba a cambiar 'langid' por 'lang' aquí debajo
			if opts.langid != selid:
				opts.langid = selid
				changed = True
