...
class PrimDirsListFrame(wx.Frame):
	...
	def __init__(self, parent, chrt, options, pds, title):
		...
		self.ID_Start = 10
		self.tb.AddTool(10, mtexts.txts["Start"], tostart_bmp, shortHelp=mtexts.txts["Start"], kind=wx.ITEM_NORMAL)
		self.Bind(wx.EVT_TOOL, self.OnStart, id=self.ID_Start)

		self.ID_Back = 20
		self.tb.AddTool(20, mtexts.txts["Back"], back_bmp, shortHelp=mtexts.txts["Back"], kind=wx.ITEM_NORMAL)
		self.Bind(wx.EVT_TOOL, self.OnBack, id=self.ID_Back)

		self.ID_Forward = 30
		self.tb.AddTool(30, mtexts.txts["Forward"], forward_bmp, shortHelp=mtexts.txts["Forward"], kind=wx.ITEM_NORMAL)
		self.Bind(wx.EVT_TOOL, self.OnForward, id=self.ID_Forward)

		self.ID_End = 40
		self.tb.AddTool(40, mtexts.txts["End"], toend_bmp, shortHelp=mtexts.txts["End"], kind=wx.ITEM_NORMAL)
