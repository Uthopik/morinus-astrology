...
class ProfDlg(wx.Dialog):
	...
	def __init__(self, parent, radixjd, radixplace):
		...
		# CORRECCIÓN: Eliminamos el objeto 'pre' y usamos el constructor directo
		...
		# CORRECCIÓN: FlexGridSizer necesita los argumentos de gap (0, 0)
		...
		fgsizer_t = wx.FlexGridSizer(1, 3, 0, 0) # Gap corregido
		...
		fgsizer_t.Add(vsizer, 0, wx.ALIGN_LEFT|wx.ALL, 5)
		...
		timesizer.Add(fgsizer_t, 0, wx.GROW|wx.ALL, 5)
	...
	def onOK(self, event):
		if (self.Validate() and self.sdate.Validate()):
			if util.checkDate(int(self.year.GetValue()), int(self.month.GetValue()), int(self.day.GetValue())):
				...
				# Comprobamos JD para asegurar que la profecha es posterior al nacimiento
