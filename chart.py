...
class Time:
	...
	def __init__(self, year, month, day, hour, minute, second, bc, cal, zt, plus, zh, zm, daylightsaving, place, full = True): #zt is zonetime, zh is zonehour, zm is zoneminute, full means to calculate everything e.g. FixedStars, MidPoints, ...
		...
		if zt == Time.LOCALAPPARENT:#LAT
			res_te = astrology.swe_time_equ(self.jd)
			te = res_te[0] # Extraemos solo el primer valor numérico de la tupla
			self.jd += te #LMT
...
class Chart:
	...
	def __init__(self, name, male, time, place, htype, notes, options, full = True, proftype = 0, nolat=False):
		...
		self.obl, serr = astrology.swe_calc(time.jd+d, astrology.SE_ECL_NUT, 0)
	...
	def create(self):
		...
		self.houses = houses.Houses(self.time.jd, hflag, float(self.place.lat), float(self.place.lon), self.options.hsys, self.obl, self.options.ayanamsha, self.ayanamsha)

		self.raequasc, declequasc, *sobras = astrology.swe_cotrans(self.houses.ascmc[houses.Houses.EQUASC], 0.0, 1.0, -self.obl)
		self.planets = planets.Planets(self.time.jd, self.options.meannode, pflag, self.place.lat, self.houses.ascmc2, self.raequasc, self.nolat, self.obl)
		...
		self.fortune = fortune.Fortune(self.options.lotoffortune, self.houses.ascmc2, self.raequasc, self.planets, self.obl, self.place.lat, abovehor)
...
# ###########################################
			self.munfortune = munfortune.MundaneFortune(self.houses.ascmc2, self.planets, self.obl, self.place.lat)
			...
			self.fixstars = fixstars.FixStars(self.time.jd, fsflag, self.options.fixstars, self.obl)
			self.midpoints = midpoints.MidPoints(self.planets)
			# Si la casilla está marcada, dst vale 1.0. Si no, vale 0.0.
			total_offset = self.time.zt + self.time.daylightsaving
			self.riseset = riseset.RiseSet(self.time.jd, self.time.cal, self.place.lon, self.place.lat, self.place.altitude, self.planets, total_offset)
			self.zodpars = zodpars.ZodPars(self.planets, self.obl)
			self.antiscia = antiscia.Antiscia(self.planets.planets, self.houses.ascmc, self.fortune.fortune, self.obl, self.options.ayanamsha, self.ayanamsha)
			self.antzodpars = antzodpars.AntZodPars(self.antiscia.plantiscia, self.antiscia.plcontraant, self.obl)
			self.almutens = almutens.Almutens(self)
			if self.options.pdcustomer:
				self.cpd = customerpd.CustomerPD(self.options.pdcustomerlon[0], self.options.pdcustomerlon[1], self.options.pdcustomerlon[2], self.options.pdcustomerlat[0], self.options.pdcustomerlat[1], self.options.pdcustomerlat[2], self.options.pdcustomersouthern, self.place.lat, self.houses.ascmc2, self.obl, self.raequasc)
			if self.options.pdcustomer2:
				self.cpd2 = customerpd.CustomerPD(self.options.pdcustomer2lon[0], self.options.pdcustomer2lon[1], self.options.pdcustomer2lon[2], self.options.pdcustomer2lat[0], self.options.pdcustomer2lat[1], self.options.pdcustomer2lat[2], self.options.pdcustomer2southern, self.place.lat, self.houses.ascmc2, self.obl, self.raequasc)
	...
	def setNodes(self):
		pflag = astrology.SEFLG_SWIEPH+astrology.SEFLG_SPEED
		self.planets = planets.Planets(self.time.jd, self.options.meannode, pflag, self.place.lat, self.houses.ascmc2, self.raequasc, self.obl)
...
#		abovehorizon = self.planets.planets[planets.Planets.SUN].speculums[planets.Planet.PLACIDIAN].abovehorizon
	...
	def dignity(self, pid):
		...
		if pid < astrology.SE_PLUTO+1:
			isdom = self.options.dignities[pid][0][int(sign)]
			isexal = self.options.dignities[pid][1][int(sign)]
	...
	def calcFixStarAspMatrix(self):
		...
		for i in range(num):
			...
			# 1. Sacamos el nombre y el orbe de forma segura
			s_name = self.fixstars.data[i][fixstars.FixStars.NOMNAME]
			orb_fix = self.options.fixstars.get(s_name, 0.0)

			# 2. Convertimos la longitud a número (float) por si viene como texto
			try:
				lon_estrella = float(self.fixstars.data[i][fixstars.FixStars.LON])
			except:
				lon_estrella = 0.0 # Si falla la conversión, ponemos 0 para no romper el programa

			# 3. Calculamos val1 y val2 usando la longitud convertida
			val1 = lon_estrella + orb_fix
			val2 = lon_estrella - orb_fix
		...
		for i in range(num):
			...
			# 1. Sacamos el nombre y el orbe de forma segura (.get evita el KeyError)
			s_nom = self.fixstars.data[i][fixstars.FixStars.NOMNAME]
			orb_fix = self.options.fixstars.get(s_nom, 0.0)

			# 2. Convertimos la longitud a número (float) para evitar el TypeError
			try:
				lon_f = float(self.fixstars.data[i][fixstars.FixStars.LON])
			except:
				lon_f = 0.0

			# 3. Ahora sumamos y restamos sin miedo
			val1 = lon_f + orb_fix
			val2 = lon_f - orb_fix
