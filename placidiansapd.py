	...
	def calcInterPlanetary(self, mundane):
		...
		for p in range(len(self.chart.planets.planets)):
			...
			if not mundane and self.options.subzodiacal != primdirs.PrimDirs.SZPROMISSOR and self.options.subzodiacal != primdirs.PrimDirs.SZBOTH:
				# 1. Definimos obliq (Fíjate en el espacio a la izquierda)
				obliq = self.chart.obl[0] if isinstance(self.chart.obl, (list, tuple)) else self.chart.obl
				
				# 2. Recalc zodiacals
				raprom, declprom, dist = astrology.swe_cotrans(plprom.data[planets.Planet.LONG], 0.0, 1.0, -obliq)
			...
			# 3. ¡OJO AQUÍ! Esta línea debe tener el mismo nivel de espacios que el "if" de arriba
...
#						raprom, declprom = util.getRaDecl(lon, latprom, self.chart.obl[0])
						...
						obliq = self.chart.obl[0] if isinstance(self.chart.obl, (list, tuple)) else self.chart.obl
						raprom, declprom, dist = astrology.swe_cotrans(lon, 0.0, 1.0, -obliq)
						val = math.tan(math.radians(self.chart.place.lat))*math.tan(math.radians(declprom))
						if math.fabs(val) > 1.0:
							continue
						adprom = math.degrees(math.asin(val))
...
#					raprom, declprom = util.getRaDecl(lon, pllat, self.chart.obl[0])
	...
	def getZodMDSA(self, lon, lat=0.0):
		...
		# 1. Definimos la oblicuidad de forma segura
		obliq = self.chart.obl[0] if isinstance(self.chart.obl, (list, tuple)) else self.chart.obl
		
		# 2. Usamos la nueva variable 'obliq' en el cálculo
		ra, decl, dist = astrology.swe_cotrans(lon, lat, 1.0, -obliq)
