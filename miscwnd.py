...
class MiscWnd(commonwnd.CommonWnd):
	...
	def __init__(self, parent, chrt, options, mainfr, id = -1, size = wx.DefaultSize):
		...
		self.SPACE = self.FONT_SIZE // 2  # División entera
		...
		# Forzamos a que el tamaño de la ventana sea entero
	...
	def drawBkg(self):
		...
		if self.bw: tableclr = (0, 0, 0)

		txtclr = self.options.clrtexts
		if self.bw: txtclr = (0, 0, 0)

		# Creamos la imagen con dimensiones enteras
		img = Image.new('RGB', (int(self.WIDTH), int(self.HEIGHT)), self.bkgclr)
		draw = ImageDraw.Draw(img)

		BOR = int(commonwnd.CommonWnd.BORDER)

		# Grid - Usamos divisiones enteras //
		x, y = BOR, BOR
		...
		# Data
		SIDTIME, OBL, JD, VERTEX, EQUATASC = 0, 1, 2, 3, 4
		txts = (mtexts.txts['SidTime'], mtexts.txts['OblEcl'], mtexts.txts['JulianDay'], mtexts.txts['Vertex'], mtexts.txts['EquAsc'])
		data = (self.chart.houses.ascmc[houses.Houses.ARMC]/15.0, self.chart.obl, self.chart.time.jd, self.chart.houses.ascmc[houses.Houses.VERTEX], self.chart.houses.ascmc[houses.Houses.EQUASC])
		
		for i in range(self.LINE_NUM):
			# Medir etiquetas de la izquierda
			l, t, r, b = draw.textbbox((0, 0), txts[i], font=self.fntText)
			w, h = r - l, b - t
			draw.text((int(BOR + (self.CELL_WIDTH - w) // 2), int(BOR + self.LINE_HEIGHT * i + (self.LINE_HEIGHT - h) // 2)), txts[i], fill=txtclr, font=self.fntText)
			...
			if i == SIDTIME:
				...
				txt = (str(pos)).rjust(2)+self.deg_symbol+(str(m)).zfill(2)+"'"+(str(m)).zfill(2)+'"'
				l_t, t_t, r_t, b_t = draw.textbbox((0, 0), txt, font=self.fntText)
				w, h = (r_t - l_t) + 12, b_t - t_t # +12 para el signo
				...
				draw.text((BOR+self.CELL_WIDTH+offset+w+wsp, BOR+self.LINE_HEIGHT*i+(self.LINE_HEIGHT-hsg)/2), self.signs[sign], fill=txtclr, font=self.fntMorinus)

		# --- Table 2 (Syzygy) ---
		txts = (mtexts.txts['Syzygy'], mtexts.txts['Date2'], mtexts.txts['Longitude'])
		x = BOR
		y = BOR+self.LINE_NUM*self.LINE_HEIGHT+self.TABLE2_OFFS
		# (Las líneas de dibujo se mantienen igual)
		...
		if not self.chart.time.bc:
			...
			# Fecha de la Syzygy (GMT)
			h_sy, m_sy, s_sy = util.decToDeg(self.chart.syzygy.time.time)
			txt = str(self.chart.syzygy.time.year)+'.'+str(self.chart.syzygy.time.month).zfill(2)+'.'+str(self.chart.syzygy.time.day).zfill(2)+'.'+' '+str(h_sy).rjust(2)+':'+str(m_sy).zfill(2)+':'+str(s_sy).zfill(2)
			l, t, r, b = draw.textbbox((0, 0), txt, font=self.fntText)
			w, h = (r - l) + 4, b - t # +4 de margen extra
