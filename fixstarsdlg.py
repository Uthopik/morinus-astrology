	...
	def read(self):
		...
		try:
			...
			for i in range(1, cnt+1):
				res = astrology.swe_fixstar_ut(str(i), self.jd, 0)
				if len(res) < 3 or not isinstance(res[1], (list, tuple)):
					return False
				name = res[0]
				dat = res[1]
				if not dat or not isinstance(dat[0], (int, float)):
					return False
...
class FixStarsDlg(wx.Dialog):
	def __init__(self, parent, options, ephepath):
		...
		# Inicialización directa y moderna sin el error de 'pre'
		wx.Dialog.__init__(self, parent, -1, mtexts.txts['FixStars'], style=wx.DEFAULT_DIALOG_STYLE | wx.DIALOG_EX_CONTEXTHELP)
