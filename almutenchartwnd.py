...
class AlmutenChartWnd(commonwnd.CommonWnd):
	...
	def __init__(self, parent, chrt, options, mainfr, id = -1, size = wx.DefaultSize):
		...
		if self.options.useaccidental:
			...
			self.ACCIDENTAL_Y_OFFSET = int(self.LINE_HEIGHT//2)
		...
		for i in range(num):
			...
			for j in range(subnum):
				...
				if pid != -1:
					...
					aux[j][0] = pid
					aux[j][1] = sco
					aux[j][2] = wpl+wsp+w + 5 # Margen extra
					if mwidth != 0:
						mwidth += wsp
					mwidth += aux[j][2]
				else:
			...
			for j in range(subnum):
				if aux[j][0] != -1:
					...
					draw.text((x+(self.DEGREEWINS_CELL_WIDTH-(mwidth))//2+prev, y+i*self.LINE_HEIGHT+(self.LINE_HEIGHT-h_score)//2), pltxt, fill=clr, font=self.fntMorinus)
					draw.text((x+(self.DEGREEWINS_CELL_WIDTH-(mwidth))//2+prev+wpl+wsp, y+i*self.LINE_HEIGHT+(self.LINE_HEIGHT-h_score)//2), txt_score, fill=txtclr, font=self.fntText)
	...
	def drawLong(self, draw, x, y, lon, clr):
		...
		# El +12 mágico para que no se pisen
