...
class MundaneChart:
	...
	def __init__(self, chrt, size, opts, bw, planetaryday=True, chrt2 = None):
		...
		# Forzamos int() en todos los tamaños de fuente para evitar el error de float
		self.fntMorinus = ImageFont.truetype(common.common.symbols, int(self.symbolSize+1))
		self.fntSmallMorinus = ImageFont.truetype(common.common.symbols, int(self.smallsymbolSize+1))
		self.fntAspects = ImageFont.truetype(common.common.symbols, int(1+self.symbolSize // 2))
		self.fntText = ImageFont.truetype(common.common.abc, int(1+self.symbolSize // 2))
		self.fntSmallText = ImageFont.truetype(common.common.abc, int(1+self.symbolSize // 4))
		self.fntBigText = ImageFont.truetype(common.common.abc, int(self.symbolSize+1))
		self.fntMorinus2 = ImageFont.truetype(common.common.symbols, int(1+self.symbolSize // 4 * 3))
		self.deg_symbol = u'\u00b0'
...
#			offs -= 3*MundaneChart.DEG30
	...
	def drawPlanets(self, chrt, pshift, rPlanet, rRetr, outer=False):
		...
		for i in range(planets.Planets.PLANETS_NUM+1):
			...
			xmp = 0.0
			# --- CÁLCULO DE POSICIÓN MUNDANA ---
			if i < planets.Planets.PLANETS_NUM:
				if self.chart.options.primarydir in (primdirs.PrimDirs.PLACIDIANSEMIARC, primdirs.PrimDirs.PLACIDIANUNDERTHEPOLE):
			...
			# Coordenadas base
			angle = math.pi + math.radians(-xmp - pshift[i])
			x = cx + math.cos(angle) * rPlanet
			y = cy + math.sin(angle) * rPlanet
			...
			if not self.bw:
				if self.options.useplanetcolors:
					objidx = i
					if i >= planets.Planets.PLANETS_NUM-1: objidx -= 1
					clr = self.options.clrindividual[objidx]
				else:
					clr = clrs[chrt.dignity(i)] if i < planets.Planets.PLANETS_NUM else self.options.clrperegrin

			txtpl = common.common.Planets[i] if i < planets.Planets.PLANETS_NUM else common.common.fortune

			# Dibujar Símbolo (usando int() para evitar errores de redondeo)
			self.draw.text((int(x - self.symbolSize/2), int(y - self.symbolSize/2)), txtpl, fill=clr, font=self.fntMorinus)

			# Retrogradación / Estacionario
			if i < planets.Planets.PLANETS_NUM:
				speed = chrt.planets.planets[i].data[planets.Planet.SPLON]
				if speed <= 0.0:
					t = 'R' if speed < 0.0 else 'S'
					xr = cx + math.cos(angle) * rRetr	
					yr = cy + math.sin(angle) * rRetr
					self.draw.text((int(xr - self.symbolSize/8), int(yr - self.symbolSize/8)), t, fill=clr, font=self.fntSmallText)

			if not outer and self.options.positions:
				# --- POSICIÓN (GRADOS Y MINUTOS) ---
				(d, m, s) = util.decToDeg(xmp)
				str_d, str_m = str(d), str(m).zfill(2)

				# CORRECCIÓN: Sustitución de textsize por textbbox
				bb_d = self.draw.textbbox((0, 0), str_d, font=self.fntText)
				wdeg, hdeg = bb_d[2] - bb_d[0], bb_d[3] - bb_d[1]
				
				# Posición del texto de grados
				xp = cx + math.cos(angle) * self.rPos
				yp = cy + math.sin(angle) * self.rPos	
				
				xdeg, ydeg = int(xp - wdeg/2), int(yp - hdeg/2)
				self.draw.text((xdeg, ydeg), str_d, fill=clrpos, font=self.fntText)
				
				# Minutos al lado de los grados
				self.draw.text((int(xdeg + wdeg), ydeg), str_m, fill=clrpos, font=self.fntSmallText)
	...
	def doShift(self, p1, p2, pshift, order, mixed, rPlanet, forward = False):
		...
		# Función auxiliar para calcular X e Y rápido
		def get_coords(idx_p, p_val):
			angle = math.pi + math.radians(asc - order[idx_p] - pshift[mixed[idx_p]])
			return cx + math.cos(angle) * rPlanet, cy + math.sin(angle) * rPlanet

		x1, y1 = get_coords(p1, order[p1])
		x2, y2 = get_coords(p2, order[p2])

		# --- CORRECCIÓN DE GETSIZE A TEXTBBOX ---
		def get_dim(idx_m):
			txt = common.common.Planets[idx_m] if idx_m < planets.Planets.PLANETS_NUM else common.common.fortune
			bbox = self.fntMorinus.getbbox(txt) # getbbox es más directo que textbbox si tienes el objeto font
			# bbox es (left, top, right, bottom)
			return float(bbox[2] - bbox[0]), float(bbox[3] - bbox[1])

		w1, h1 = get_dim(mixed[p1])
		w2, h2 = get_dim(mixed[p2])
