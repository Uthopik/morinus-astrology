...
class GraphChart2:
	...
	def __init__(self, chrt, size, opts, bw, planetaryday=True, chrt2 = None):
		...
		self.fntMorinusSigns = ImageFont.truetype(common.common.symbols, int(self.signSize+1))
		self.fntText = ImageFont.truetype(common.common.abc, int((self.symbolSize/2))+1)
		self.fntAntisText = ImageFont.truetype(common.common.abc, int(self.symbolSize+1))
		self.fntSmallText = ImageFont.truetype(common.common.abc, int((self.symbolSize/2))+1)
		self.fntRetr = ImageFont.truetype(common.common.symbols, int((self.symbolSize/2))+1)
		self.fntSmallText2 = ImageFont.truetype(common.common.abc, int((self.symbolSize/3))+1)
		self.fntSmallTextOuter = ImageFont.truetype(common.common.abc, int((self.symbolSize/4))+1)
		self.fntBigText = ImageFont.truetype(common.common.abc, int((self.symbolSize/4*3))+1)
		self.fntMorinus2 = ImageFont.truetype(common.common.symbols, int((self.symbolSize/4*3))+1)
		self.deg_symbol = u'\u00b0'
	...
	def drawCircles(self):
		...
		if self.chart2 != None and self.options.houses:
			...
			pen = wx.Pen(clr, 1)
			self.bdc.SetPen(pen)
			self.bdc.DrawCircle(int(cx), int(cy), int(self.rOuterMax))
			self.bdc.DrawCircle(int(cx), int(cy), int(self.rOuterHouse))
	...
	def drawPlanets(self, chrt, pshift, rPlanet, rRetr, outer=False):
		...
		for i in range (planets.Planets.PLANETS_NUM+1):
			...
			if not outer:
				...
				degtxt = str(d)+self.deg_symbol
				left, top, right, bottom = self.draw.textbbox((0, 0), degtxt, font=self.fntText)
				wdeg, hdeg = right - left, bottom - top
				...
				mintxt = str(m)+"'"
				left, top, right, bottom = self.draw.textbbox((0, 0), mintxt, font=self.fntSmallText2)
				wmint, hmint = right - left, bottom - top
...
#							t = 'R'
						...
						bbox = self.draw.textbbox((0, 0), t, font=rfnt)
						wdeg = bbox[2] - bbox[0] # derecha - izquierda
						hdeg = bbox[3] - bbox[1] # abajo - arriba
	...
	def doShift(self, p1, p2, pshift, order, mixed, rPlanet, forward = False):
		...
		if mixed[p1] < planets.Planets.PLANETS_NUM:
			# Usamos GetTextExtent pasando el string del planeta
			w1, h1 = self.bdc.GetTextExtent(common.common.Planets[mixed[p1]])
		else:
			w1, h1 = self.bdc.GetTextExtent(common.common.fortune)
			
		w2, h2 = 0, 0
		if mixed[p2] < planets.Planets.PLANETS_NUM:
			w2, h2 = self.bdc.GetTextExtent(common.common.Planets[mixed[p2]])
		else:
			w2, h2 = self.bdc.GetTextExtent(common.common.fortune)
