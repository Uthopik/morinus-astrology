...
#		self.fntSymbol = ImageFont.truetype(common.common.symbols, 3*self.FONT_SIZE//2)
	...
	def drawBkg(self):
		...
		for i in range(chart.Chart.SIGN_DEG+1):
			if i == 0 or i % 5 == 0:
				txt = str(i)
				# CORRECCIÓN 1: Sustitución de textsize por textbbox
		...
		for i in range(num):
			...
			# CORRECCIÓN 2: Sustitución de textsize por textbbox
	...
	def arrange(self):
		...
		if order[0]+bshift[0] < BOR:
			...
			for i in range(len(order)-1):
				w1, h1 = 0.0, 0.0
				tidx = planets.Planets.PLANETS_NUM-1
				# --- CORRECCIÓN GETSIZE -> GETBBOX ---
				midx1 = mixed[i]
				if midx1 < tidx:
					bbox = self.fntMorinus.getbbox(common.common.Planets[midx1])
					w1, h1 = bbox[2]-bbox[0], bbox[3]-bbox[1]
				elif midx1 == tidx:
					bbox = self.fntMorinus.getbbox(common.common.fortune)
					w1, h1 = bbox[2]-bbox[0], bbox[3]-bbox[1]
				elif midx1 == tidx+1:
					bbox = self.fntText.getbbox(self.ascmctxt[0])
					w1, h1 = bbox[2]-bbox[0], bbox[3]-bbox[1]
				elif midx1 == tidx+2:
					bbox = self.fntText.getbbox(self.ascmctxt[1])
					w1, h1 = bbox[2]-bbox[0], bbox[3]-bbox[1]

				w2, h2 = 0.0, 0.0
				midx2 = mixed[i+1]
				if midx2 < tidx:
					bbox = self.fntMorinus.getbbox(common.common.Planets[midx2])
	...
	def doShift(self, b1, b2, bshift, order, mixed, forward = False):
		...
		# --- CORRECCIÓN GETSIZE -> GETBBOX ---
		if mb1 < tidx:
			bbox = self.fntMorinus.getbbox(common.common.Planets[mb1])
			w1, h1 = bbox[2]-bbox[0], bbox[3]-bbox[1]
		elif mb1 == tidx:
			bbox = self.fntMorinus.getbbox(common.common.fortune)
			w1, h1 = bbox[2]-bbox[0], bbox[3]-bbox[1]
		...
		mb2 = mixed[b2]
		if mb2 < tidx:
			bbox = self.fntMorinus.getbbox(common.common.Planets[mb2])
			w2, h2 = bbox[2]-bbox[0], bbox[3]-bbox[1]
		elif mb2 == tidx:
			bbox = self.fntMorinus.getbbox(common.common.fortune)
			w2, h2 = bbox[2]-bbox[0], bbox[3]-bbox[1]
