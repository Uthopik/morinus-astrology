...
#		self.parent.SetMinSize((400,300))
	...
	def drawBkg(self):
		...
		self.txtSymbolSize = min(self.signSize, int(self.monthSize)/3)
		self.signSymbolSize = self.txtSymbolSize

		self.fntPlanets = ImageFont.truetype(common.common.symbols, int(self.planetSymbolSize+1))
		self.fntSigns = ImageFont.truetype(common.common.symbols, int(self.signSymbolSize+1))
		self.fntTxt = ImageFont.truetype(common.common.abc, int(self.txtSymbolSize+1))
...
#		x2 = self.w-self.BORDER
		...
		for i in range(chart.Chart.SIGN_NUM):
			left, top, right, bottom = draw.textbbox((0, 0), self.signs[i], font=self.fntSigns)
			wsym, hsym = right - left, bottom - top
			draw.text((x,y-self.signSize//2-hsym//2-i*self.signSize), self.signs[i], fill=signsclr, font=self.fntSigns)

		#year
		txt = str(self.year)
		left, top, right, bottom = draw.textbbox((0, 0), txt, font=self.fntTxt)
		wtxt, htxt = right - left, bottom - top
		x = 2*self.BORDER+self.signSymbolSize+self.spaceSize
		y = self.h-4*self.BORDER+self.spaceSize
		offs = int((self.monthSize-wtxt)/2)
		...
		for i in range(mnum):
			...
			l, t, r, b = draw.textbbox((0, 0), txt, font=self.fntTxt)
			wtxt, htxt = r - l, b - t
			offs = int((self.monthSize-wtxt)/2)
		...
		for pl in range(plsnum):
			if pl != 1: #Moon excepted
				...
				if not self.bw:
					plsclr = self.options.clrindividual[pl]
				left, top, right, bottom = draw.textbbox((0, 0), common.common.Planets[pl], font=self.fntPlanets)
				wsym, hsym = right - left, bottom - top
		...
		if len(plstop) != 0:
			...
			for pl in range(plstopnum):
				...
				if not self.bw:
					plsclr = self.options.clrindividual[plstopids[pl]]
				left, top, right, bottom = draw.textbbox((0, 0), plstop[pl], font=self.fntPlanets)
				wsym, hsym = right - left, bottom - top
		...
		if len(plsbottom) != 0:
			...
			for pl in range(plsbottomnum):
				...
				if not self.bw:
					plsclr = self.options.clrindividual[plsbottomids[pl]]
				left, top, right, bottom = draw.textbbox((0, 0), plsbottom[pl], font=self.fntPlanets)
				wsym, hsym = right - left, bottom - top
	...
	def arrange(self, pls, plpixelpos, smallerBOR, greaterBOR):
		...
		if order[0]+bshift[mixed[0]] < BOR:
			...
			for i in range(num-1):
				l1, t1, r1, b1 = self.fntPlanets.getbbox(pls[i])
				w1, h1 = r1 - l1, b1 - t1
				l2, t2, r2, b2 = self.fntPlanets.getbbox(pls[i+1])
				w2, h2 = r2 - l2, b2 - t2

				x1 = int(order[i]+bshift[mixed[i]])
				x2 = int(order[i+1]+bshift[mixed[i+1]])
		...
		if order[lenord]+bshift[mixed[lenord]] > greaterBOR:
			...
			for i in range(lenord, 0, -1):
				l1, t1, r1, b1 = self.fntPlanets.getbbox(pls[i-1])
				w1, h1 = r1 - l1, b1 - t1
				l2, t2, r2, b2 = self.fntPlanets.getbbox(pls[i])
				w2, h2 = r2 - l2, b2 - t2

				x1 = int(order[i-1]+bshift[mixed[i-1]])
				x2 = int(order[i]+bshift[mixed[i]])
	...
	def doShift(self, b1, b2, pls, bshift, order, mixed, forward = False):
		...
		x1 = int(order[b1]+bshift[mixed[b1]])
		x2 = int(order[b2]+bshift[mixed[b2]])

		left1, top1, right1, bottom1 = self.fntPlanets.getbbox(pls[mixed[b1]])
		w1, h1 = right1 - left1, bottom1 - top1
		
		left2, top2, right2, bottom2 = self.fntPlanets.getbbox(pls[mixed[b2]])
